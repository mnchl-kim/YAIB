...
#   -m  MODEL         lgbm | gru | latentode | both   (default: both)
...
run_one() {
  ...
  if [ "$model" = "GRU" ] || [ "$model" = "LatentODE" ]; then
...
case "${MODEL,,}" in
  ...
  both|all)
    ...
    ;;
  *)
    echo "Unknown MODEL: $MODEL (사용 가능: lgbm | gru | latentode | both)" >&2
    exit 1
    ;;
esac
